...
contract ApplyPolicy is Script {
    ...
    function run() external {
        ...
        string memory policyVersion = vm.parseJsonString(
            json,
            ".meta.policyVersion"
        );
    ...
    // Uses index-based iteration instead of vm.parseJsonUint(".length")
    function _applyTargets(PolicyGuard guard, string memory json) internal {
        for (uint256 i = 0; ; i++) {
            string memory basePath = string.concat(
                ".policyBundle.targets[",
                vm.toString(i),
                "]"
            );
            // Try reading the label — if the index doesn't exist, Foundry reverts
            try
                vm.parseJsonString(json, string.concat(basePath, ".label"))
            returns (string memory label) {
                string memory addrTemplate = vm.parseJsonString(
                    json,
                    string.concat(basePath, ".address")
                );
    ...
    function _applySelectors(PolicyGuard guard, string memory json) internal {
        for (uint256 i = 0; ; i++) {
            ...
            try
                vm.parseJsonString(json, string.concat(basePath, ".label"))
            returns (string memory label) {
                string memory targetTemplate = vm.parseJsonString(
                    json,
                ...
                bytes4 selector = bytes4(
                    vm.parseJsonBytes(
                        json,
                        string.concat(basePath, ".selector")
                    )
                );
    ...
    function _applyTokens(PolicyGuard guard, string memory json) internal {
        for (uint256 i = 0; ; i++) {
            ...
            try
                vm.parseJsonString(json, string.concat(basePath, ".symbol"))
    ...
    function _applySpenders(PolicyGuard guard, string memory json) internal {
        for (uint256 i = 0; ; i++) {
            ...
            try
                vm.parseJsonString(json, string.concat(basePath, ".label"))
            returns (string memory label) {
                string memory tokenTemplate = vm.parseJsonString(
                    json,
                    string.concat(basePath, ".token")
                );
                address token = _resolveAddress(json, tokenTemplate);
                string memory spenderTemplate = vm.parseJsonString(
                    json,
                    string.concat(basePath, ".spender")
                );
    ...
    // ─── Write Audit Log ───
    function _writeAuditLog(
        string memory network,
        string memory policyVersion,
        address guardAddr
    ) internal {
        string memory timestamp = vm.toString(block.timestamp);
        string memory logFile = string.concat(
            "logs/policy-audit/",
            timestamp,
            ".json"
        );
