...
/// @title ListingManagerV2
/// @notice Manages template listings and rent-to-mint with subscription model
...
contract ListingManagerV2 is Ownable2Step, ReentrancyGuard {
    ...
    struct ListingConfig {
        ...
        uint32 gracePeriodDays; // V2: configurable grace period (0 = use default)
    }
    ...
    /// @notice V2: SubscriptionManager for lifecycle management
    address public subscriptionManager;

    /// @notice V2: default grace period when listing doesn't specify one
    uint32 public defaultGracePeriodDays = 7;
    ...
    /// @notice V2: Set SubscriptionManager address
    function setSubscriptionManager(address _subManager) external onlyOwner {
        if (_subManager == address(0)) revert Errors.ZeroAddress();
        subscriptionManager = _subManager;
    }

    /// @notice V2: Set default grace period for new listings
    function setDefaultGracePeriodDays(uint32 _days) external onlyOwner {
        defaultGracePeriodDays = _days;
    }
    ...
    // ═══════════════════════════════════════════════════════════
    //              RENT-TO-MINT WITH SUBSCRIPTION
    // ═══════════════════════════════════════════════════════════

    /// @notice Rent by minting a dedicated instance + create subscription
    function rentToMintWithParams(
        ...
        // V2: Create subscription in SubscriptionManager
        if (subscriptionManager != address(0)) {
            uint32 graceDays = cfg.gracePeriodDays > 0
                ? cfg.gracePeriodDays
                : defaultGracePeriodDays;

            uint96 pricePerPeriod = uint96(totalCost);
    ...
    /// @notice Claim rental income (initial rent + renewal revenue)
    ...
    /// @notice V2: Set listing config (maxDays + gracePeriodDays)
    function setListingConfig(
        ...
        listingConfigs[listingId] = ListingConfig(maxDays, gracePeriodDays);
        emit ListingConfigUpdated(listingId, maxDays, gracePeriodDays);
    }

    /// @notice V1-compatible: Set listing config (maxDays only, grace = default)
    function setListingConfig(bytes32 listingId, uint32 maxDays) external {
        ...
        listingConfigs[listingId] = ListingConfig(maxDays, 0);
        emit ListingConfigUpdated(listingId, maxDays, 0);
    }
